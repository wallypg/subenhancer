...
foreach ($totalSegmentsOverCps as $segmentOverCps) {
    // echo $segmentOverCps.'<br>';
    fillEmptySpace($subtitle,$segmentOverCps,$cps);
    // if($segmentOverCps == 0) {
    //  // Primera secuencia

    // } elseif($segmentOverCps == $totalSequences-1) {
    //  // Última secuencia
    // } else {
...
function thisLineOverCps ($subtitle,$sequence,$cps) {
    if(property_exists($subtitle,$sequence) && $subtitle->$sequence->cps <= $cps) return false;
    return true;
}
...
function fillEmptySpaceBefore ($subtitle,$sequence,$cps) {
    $previousSequence = $sequence - 1;

    $missingTime = checkMissingTime($subtitle->$sequence,$cps);

    if(property_exists($subtitle,$previousSequence)) $availableTimeBefore = checkAvailableTimeBefore($subtitle,$sequence);
    ...
    if(isset($availableTimeBefore)) {
        if($availableTimeBefore<$missingTime) {
            ...
            $subtitle->$sequence->startTimeInMilliseconds = $subtitle->$previousSequence->endTimeInMilliseconds+1;  
        } else {
            // Tengo espacio para alcanzar los cps deseados
            $subtitle->$sequence->startTimeInMilliseconds -= $missingTime;
        }
    } else {
        // Primera línea
        $subtitle->$sequence->startTimeInMilliseconds -= $missingTime;
        if($subtitle->$sequence->startTimeInMilliseconds<0) $subtitle->$sequence->startTimeInMilliseconds = 0;
    }
    // Update sequence duration
    updateSequenceData($subtitle,$sequence);
    return;
}

function fillEmptySpaceAfter ($subtitle,$sequence,$cps) {
    $nextSequence = $sequence + 1;
    $missingTime = checkMissingTime($subtitle->$sequence,$cps);

    if(property_exists($subtitle,$nextSequence)) $availableTimeAfter = checkAvailableTimeAfter($subtitle,$sequence);
    
    if(isset($availableTimeAfter)) {
        if($availableTimeAfter<$missingTime) {
            // Ocupo todo el espacio que tengo disponible aunque no alcance
            $subtitle->$sequence->endTimeInMilliseconds = $subtitle->$nextSequence->startTimeInMilliseconds-1;  
        } else {
            // Tengo espacio para alcanzar los cps deseados
            $subtitle->$sequence->endTimeInMilliseconds -= $missingTime;
        }
    } else {
        // Última línea
        $subtitle->$sequence->endTimeInMilliseconds -= $missingTime;
...
function fillEmptySpace ($subtitle,$sequence,$cps) {
    // fillEmptySpaceBefore si es la primer línea o hay una línea anterior pero no supera los $cps
    if(thisLineOverCps($subtitle,$sequence-1,$cps)) {
        echo $sequence.'<br>';
        // fillEmptySpaceBefore($subtitle,$sequence,$cps);
    }
    ...
    // if(thisLineOverCps($subtitle,$sequence,$cps) && thisLineOverCps($subtitle,$sequence+1,$cps)) fillEmptySpaceAfter($subtitle,$sequence,$cps);
}
